        ...
        <div className="flex items-center gap-3">
          ...
          {!isLoading && isAuthenticated && user && (
            <>
              ...
              {/* ปุ่มไป Dashboard */}
              <Link
                href={DASHBOARD_LINK[user.role] || '/orders'}
                className="text-sm px-4 py-2 bg-gray-900 text-white rounded-xl hover:bg-gray-800 transition-colors"
              >
                {user.role === 'customer' ? 'ออเดอร์ของฉัน' : 'Dashboard'}
              </Link>

              {/* ปุ่ม Logout */}
              <button
                onClick={logout}
                className="text-sm px-4 py-2 text-gray-500 hover:text-gray-700 border border-gray-200 rounded-xl hover:bg-gray-50 transition-colors"
              >
                ออกจากระบบ
              </button>
            </>
